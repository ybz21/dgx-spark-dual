...
. "$CFG"
peer(){ ssh -o StrictHostKeyChecking=no "$SSH_USER@$WORKER_LAN_IP" "$@"; }
...
echo "== worker: 装脚本和配置 =="
tar -C "$here" -cf - glm53-ops.env glm53-rail.sh systemd/glm53-rail.service systemd/glm53-worker-boot.service \
  | peer 'mkdir -p ~/glm53-ops && tar -C ~/glm53-ops -xf -'
